...
final class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func setupUI() {
        ...
        scoreLabel.fontSize = 20; scoreLabel.fontColor = .red; addChild(scoreLabel)
        timeLabel.fontSize  = 20; timeLabel .fontColor = .red; addChild(timeLabel)
        levelLabel.fontSize = 20; levelLabel.fontColor = .red; addChild(levelLabel)
    ...
    private func layoutUI() {
        ...
        // Queue positions
        for (i, ball) in ballQueue.enumerated() {
    ...
    private func spawnBalls() {
        // Remove old target balls first
        enumerateChildNodes(withName: "//*") { node, _ in
            if let name = node.name, self.ballTextures.keys.contains(name) {
    ...
    private func setupBallQueue() {
        enumerateChildNodes(withName: "queueBall") { node, _ in node.removeFromParent() }
        ballQueue.removeAll()
        ...
        for i in 0..<queueSize {
            ...
            ball.position = CGPoint(x: size.width - 40, y: 100 + CGFloat(i) * padding)
            ball.zPosition = 10
    ...
    private func addNewBallToQueue() {
        ...
        let padding: CGFloat = 40
        ball.position = CGPoint(x: size.width - 40, y: 100 + CGFloat(ballQueue.count) * padding)
        ball.zPosition = 10
    ...
    private func shootBall(towards location: CGPoint) {
        ...
        let type = queuedBall.userData?["type"] as? String ?? queuedBall.name ?? ""
        ...
        body.applyImpulse(CGVector(dx: (dx / mag) * speed, dy: (dy / mag) * speed))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if gameActive {
            ...
            cannon.run(rotate) { self.shootBall(towards: location) }
        }
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        // Ground → remove projectile
        if let projBody = bodies.first(where: { $0.categoryBitMask & PhysicsCategory.projectile != 0 }),
           bodies.contains(where: { $0.categoryBitMask & PhysicsCategory.ground != 0 }) {
            projBody.node?.removeFromParent()
            return
        }

        // Projectile ↔ Target
        guard let projBody = bodies.first(where: { $0.categoryBitMask & PhysicsCategory.projectile != 0 }),
              let tgtBody  = bodies.first(where: { $0.categoryBitMask & PhysicsCategory.target != 0 }),
              let projNode = projBody.node as? SKSpriteNode,
              let tgtNode  = tgtBody.node as? SKSpriteNode
        else { return }

        let projType = projNode.userData?["type"] as? String ?? ""
        if projType == tgtNode.name {
            // Match → remove both
            projNode.removeFromParent()
            tgtNode.removeFromParent()
            score += 10
        } else {
            projNode.removeFromParent()
            // No match → stop projectile
      }
    }
    ...
    private func restartGame() {
        ...
        enumerateChildNodes(withName: "//*") { node, _ in
            if node.name?.contains("ball") == true || node.name == "projectile" {
                node.removeFromParent()
            }
        }
